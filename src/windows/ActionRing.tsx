...
export default function ActionRing() {
  ...
  useEffect(() => {
    ...
    const unlistenShow = listen("ring:show", () => {
      setIsVisible(false); loadData(); setHoveredMainId(null); setHoveredChildId(null); setClickedId(null);
      summonTimeRef.current = Date.now(); setCenter({ x: 400, y: 400 }); setAnimKey(k => k + 1);
  ...
  return (
    <div className="absolute inset-0 w-[800px] h-[800px] select-none overflow-hidden" style={{ background: "transparent" }} onClick={() => { if(hoveredMainId || hoveredChildId) executeAction(hoveredMainId, hoveredChildId); else invoke("hide_action_ring").catch(console.error); }}>
      <div key={animKey} className={`w-full h-full relative ${isVisible ? `opacity-100 ${animClass}` : "opacity-0 transition-none duration-0"}`}>
        ...
        {slices.map((folder, folderIdx) => {
          ...
          // จุดศูนย์กลางของโฟลเดอร์แม่ สำหรับดึงลูกๆ กลับไปซ่อน
          ...
          return (
            <Fragment key={`folder-anim-${folder.id}`}>
              {children.map((child, i) => {
                ...
                return (
                  <div key={child.id} className={isFolderOpen ? "pointer-events-auto" : "pointer-events-none"}>
